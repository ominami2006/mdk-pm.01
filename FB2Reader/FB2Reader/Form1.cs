...
namespace FB2Reader
{
    public partial class Form1 : Form
    {
        ...
        // Updated page dimensions for the reader
        private readonly Size ReaderPageDimensions = new Size(760, 580); // Width, Height (increased height)
        ...
        private async void OpenFileButton_Click(object sender, EventArgs e)
        {
            ...
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    success = await Task.Run(() => book.LoadAndProcessFile(filePath, ReaderPageDimensions));

                    if (success && book.Chapters.Any() && book.TotalPagesInBook > 0)
                    {
                        Form3 readerForm = new Form3(book, ReaderPageDimensions);
                        this.Hide();
